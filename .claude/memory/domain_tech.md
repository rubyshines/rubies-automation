...
**Daily Sync Runner (daily-sync-all.js):** runs its sub-pipelines sequentially — SEO, Email, Reviews, Products, Inventory, Orders, Customers, Conversations, Finance, Shipping Zones, Fulfillment Costs, Delivery Times, Gmail Intelligence, Gmail CS Intake, Gmail Inbox Cleanup, Gmail Watch Renewal, Gmail Read State, Ticket Reconciliation, Dead-letter Replay, AI Cost Rollup, Advisor Edit Rate, Closeness Judge, Auto-send Shadow, plus monthly (1st-only) AI Pricing Check + Bill Reconciliation (see `daily-sync-all.js` for the authoritative set). Sends the consolidated **daily ops digest** email (subject "RUBIES Daily Sync") with results.
...
- **An MCP handler returns the content envelope, never a bare value, and a static test asserts it.** The server hands a handler's return value straight to the client, so a string fails result validation and a plain data object renders as nothing — in both cases the tool is dead from Claude Code while its CLI twin works, and the module still parses, imports and unit-tests clean. Same unchecked-wiring family as inline `onclick` handlers and lazily destructured requires: no compile-time reference joins the handler to the protocol, so `customer-service/test/mcpToolShape.test.js` stands in for one.
- **A reply parser's "visible text" is not the sender's text — it drops their signature too (2026-09-11):** `stripQuotedContent` (gmail-management/lib/gmailSync.js) is the one body cleaner for every inbound path — B2B correlation and both CS intakes. `email-reply-parser`'s `getVisibleText()` hides quoted chains AND signature blocks under one flag, so every stored `email_messages.body_text` in the history of this book ends at the last sentence and loses how the person signed it. It now keeps every non-quoted fragment. Nothing surfaced it for years because a truncated body still reads as a complete message; it was found by noticing that contacts who sign every email had no name on file. Stored bodies stay truncated until re-synced.
