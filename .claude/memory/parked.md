...
- Type: bug
- Priority: high
- Notes: Fed a genuine Lightspeed-generated purchase order (CSV with a header row, one line per SKU), the tool reported `format: "simple_text"` rather than detecting the CSV, then produced confidently wrong output: it matched the HEADER ROW itself to a $50 gift card, reset every quantity to 1, and — the dangerous part — resolved plus sizes to YOUTH NUMERIC sizes (`XL` → Black/4, `2XL` → Black/6, so a 2X bra came back as a size-6 kids bra). Nothing errored; `errors: []`. The standing rule in domain_b2b_sales.md is "never manually parse CSV, always pass raw to `parse_wholesale_input`", so this sits directly in the path of every wholesale order and the failure mode looks plausible enough to survive a skim. Worked around on the She Bop order by resolving each exact SKU against the catalog instead (their vendor SKUs happened to be ours verbatim, which will not generally hold).
- Resume when: next wholesale order arrives as a CSV/PO rather than free text. Fix wants the CSV detector to recognise a per-line (not matrix) PO layout, a header-row skip, and size resolution that never silently crosses the adult/youth boundary.
...
- Parked: 2026-08-10
- Resolved: 2026-08-12
- Type: bug (closed, no code change needed)
- Domains: tech, finance
- Outcome: **The ledger is accurate.** Reconciled against the actual Anthropic bill via the Admin API cost report (`lib/billReconcile.js` uses the same endpoint): over 30 days the ledger read $497.01 against $493.52 billed — **+0.7%**, well inside the 12% alarm threshold — and it agrees day by day and model by model (opus-4-8 $383.70 vs $379.75, sonnet-5 exact, opus-5 $41.12 vs $41.62).
- Where the 41% came from: the eval's "$29.24 drafting" figure is, to the cent, the ledger's own `advisor_eval_judge` cost for 2026-08-04. Judge cost was compared against drafting cost — two different populations, not a pricing defect. The cache-write pricing suspicion was unfounded: recomputing `cs_advisor` from stored token counts at the `aiPricing` rates reproduces the stored total exactly ($311.21).
- Lesson worth keeping: **reconcile against the vendor bill, not against another one of our own estimates.** The original diagnosis compared two internal numbers and concluded the ledger was broken; one call to the cost report would have settled it in minutes. The monthly `billReconcile` only runs on the 1st, which is why 30 days can pass with no check.
...
- Domains: cs
- Priority: high
- **2026-08-18, the operator half of the first-contact-forward case is fixed; the ADVISOR half below is not.** A customer forwarded their order confirmation under three lines of their own and the operator saw a blank message. That one was not a bad heuristic — the 08-05 html-vs-plain guard already handled it and the email intake card simply never called it (see domain_cs.md). So it narrows this entry rather than adding to it: the render side now recovers the customer's words and offers the forwarded block behind a toggle. What the AI pass is still needed for is the required scope added 08-10 — making that block usable BACKGROUND for the advisor on first contact, which no render-side fix touches.
