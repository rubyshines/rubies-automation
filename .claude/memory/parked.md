...
## 2026-07 comprehensive review — remaining remediation (Phase 4 only)
- Parked: 2026-07-08
- Last touched: 2026-07-08
- Type: refactor (planned)
- Domains: tech, cs, finance, marketing
- Plan: .claude/plans/can-you-write-a-serialized-flute.md
- Notes: The 2026-07-02 whole-codebase multi-agent review found 183 confirmed issues. **ALL 25 highs are fixed + deployed** across 6 merged PRs (#56 security, #58 finance/reports/gmail data-integrity, #59 money/idempotency, #61 intake/sync, #62 discovery/marketing, #63 shared Supabase helpers + last high). **Phases 2b, 7, and 8 shipped 2026-07-08** (PRs #65, #67, #68): all `fetchAll` copies migrated onto shared `fetchAllPaginated` + full truncation sweep (2b); DRY consolidations (theme publisher, Gorgias drift core, feedback classification, warehance shipping-method resolver, dashboard advisor-input builder) + bundle-aware split_shipment allocation, intake auto-close atomic claim, refunded-line-item visibility, and a live advisor-tool crash fix (sizingEngine exports) (7); ~30 medium/low correctness fixes incl. Gorgias message pagination, atomic draft-action appends via new `append_draft_action` RPC, B2B email threading (real RFC Message-IDs + thread_id), fulfillment-webhook dedup, finance snapshot filtering, Klaviyo retry DRY (8). Full finding list + per-PR manual-test checklist live in `temp-analysis-data/` (GITIGNORED — local to Jamie's machine; regenerate via the review workflow if lost, script at `.../workflows/scripts/rubies-comprehensive-review-*.js`). Shared primitives in `shared/supabaseClient.js`: `fetchAllPaginated`, `readMany`, `readOne`.
  **Phase 6 also shipped 2026-07-08** (PR #70): "Always use Opus" replaced with the balanced "Right model for the task" policy in CLAUDE.md + feedback_technical_rules.md; shippingInfo/tracking-analyzer polish + addressHoldResolver release upgraded to Opus; b2b-discovery analyzer deliberately kept Haiku (tooling not yet exercised — see domain_b2b_sales.md); qcResults' stale pinned opus-4-6 → MODELS.OPUS; dead raw-SDK clients deleted.
  Phase left:
  **4 (HIGH RISK, the last one)** — extract shared `runToolLoop` from the 5 duplicated tool-loop bodies: `operatorAgent.js` main + shadow, `operatorAgentStandalone.js`, `aiAdvisor.js` main + shadow. Full design in the plan file "Phase 4" section: new `customer-service/lib/runToolLoop.js` owns only the core shape (call → filter tool_use → dispatch → append tool_result → repeat); divergences move behind options/callbacks (`dispatchTool`, `toolAllowlist`, `onToolResult`, `onText`, `abort`, `onApiError`). The advisor's 529-fallback + dual-mode streaming are LOAD-BEARING — they move into callbacks unchanged. Migrate incrementally, one commit per call site, simplest first: standalone → operator shadow → operator main (abort) → advisor pair (onApiError/dual-stream) last. Verify: unit tests green after each commit, `runHoldout.js` + scenario harness before/after the advisor migration, then manually exercise the dashboard operator console + a live advisor draft.
  Deliberately deferred from 7/8 (still open, small): dashboard app.js order money-summary dedup + focus-timer/notification bugs (need the dashboard running for visual verification); `conversation_history` 3-writer read-modify-write race (needs a merge-semantics decision); aiAdvisor legacy-output-mode default flip (advisor behavior — holdout first). sizingEngine dead-code deletion and b2b queueContext cadence fields have their own parked entries.
  Resume (fresh session): `/orient then continue July review remediation — Phase 4 runToolLoop`. First run `runHoldout.js` to capture the BEFORE baseline, then worktree off origin/main and follow the migration order above. Housekeeping if not yet done: remove the `review-phase6` + `memory-review-phases` worktrees; verify the `append_draft_action` migration was applied in Supabase SQL Editor (code falls back to read-modify-write until then; seedSizeCharts re-run + dashboard restart were done 2026-07-08).
