...
## 2026-07 comprehensive review — remaining remediation (Phases 2b, 4, 6, 7, 8)
- Parked: 2026-07-08
- Last touched: 2026-07-08
- Type: refactor (planned)
- Domains: tech, cs, finance, marketing
- Plan: .claude/plans/can-you-write-a-serialized-flute.md
- Notes: The 2026-07-02 whole-codebase multi-agent review found 183 confirmed issues. **ALL 25 highs are fixed + deployed** across 6 merged PRs (#56 security, #58 finance/reports/gmail data-integrity, #59 money/idempotency, #61 intake/sync, #62 discovery/marketing, #63 shared Supabase helpers + last high). Remaining = medium/low only (~158). Full finding list + per-PR manual-test checklist live in `temp-analysis-data/` (GITIGNORED — local to Jamie's machine; regenerate the findings via the review workflow if lost, script at `.../workflows/scripts/rubies-comprehensive-review-*.js`). Shared primitives already shipped in `shared/supabaseClient.js`: `fetchAllPaginated`, `readMany`, `readOne`.
  Phases left:
  **2b (low risk, do first)** — migrate the 8 duplicated `fetchAll(buildQuery,pageSize)` copies onto `fetchAllPaginated` (heads-up: `autosendConfig`/`autoactionConfig` export `fetchAll` and a test calls it with a positional pageSize — update the test or keep a thin delegating wrapper); then remaining truncation sites: dashboard stats, `seoTrends`/`seoMeta`, `inventory` tool, `accountDetail`, finance `weeklyDigest`, `blogResearch`, `seoAnalysis`, `syncProducts.detectPriceChanges`, `billReconcile` (offset w/o ORDER BY), backfill scripts, `contextBuilder` swallowed TypeError, `extractToneSamples`.
  **4 (HIGH RISK)** — extract shared `runToolLoop` from 5 copies; disturbs the advisor's 529-fallback + dual-mode streaming; run `runHoldout.js` + scenario harness before/after. Not for an autonomous pass.
  **6 (needs Jamie's input)** — rewrite "Always use Opus" → balanced model policy (CLAUDE.md + feedback_technical_rules.md); upgrade `shippingInfo` + `tracking/analyzer` customer-facing polish to Opus; DECIDE two model-tier calls: `b2b-discovery/lib/analyzer.js` (Haiku doing full prospect routing — the dismiss path has no human check) and `reports/lib/addressHoldResolver.js` (Sonnet on the address-hold release).
  **7** — DRY consolidations (publisher copy-paste, drift-detection ×3, feedback-classification ×3, size-chart dup, mapMetafields dup, dashboard front-end dups) + two deferred bug-fixes: `split_shipment` multi-line-item (reuse refundOrder's allocation helper) and the intake auto-close atomic claim (no schema — `cs_ai_drafts` already has `UNIQUE(gorgias_ticket_id, gorgias_message_id)`).
  **8** — ~40 medium/low cleanups (schema-vs-doc mismatches, off-by-one dates, Puppeteer page leak, dashboard front-end stat/notification bugs).
  Resume: read the plan file, start Phase 2b (lowest risk) or Phase 6 (needs decisions). Also enabled + verified `GMAIL_WEBHOOK_SECRET` this session (Railway + Pub/Sub `?secret`); it's in local `.env` too.
