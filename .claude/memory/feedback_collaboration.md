...
**Memory is part of shipping, not a follow-up.** When work is about to ship, prepare the memory delta (domain Key Decision, parked entries for deferred scope) and propose it as part of the pre-ship close-out — BEFORE the push — so it lands in the same commit as the code. Do NOT push the code and then ask "want me to add memory?" — that makes memory an afterthought that gets skipped, and it broke the same-commit rule above (2026-07-06 advocacy ship). The approval gate ("propose before writing") is satisfied by surfacing the one-line deltas during close-out and getting a quick yes, not by deferring the write until after the deploy. Add "is memory updated?" to the close-out sweep and answer it before pushing.

**Enforced by hook (2026-07-22):** `.claude/hooks/memory-closeout-check.js` (PreToolUse Bash) blocks any `git push` to main whose outgoing range touches no `.claude/memory/` path. Escape hatch after a genuine no-memory-needed close-out: prefix the same push command with `MEMORY_REVIEWED=1`. The block is the close-out question, asked deterministically — do not reach for the override before actually running the sweep.
