...
**Pre-Order Web Sync:** `sync_pre_orders` MCP tool + `scripts/syncPreOrders.js` CLI read the incoming-inventory sheet (us-YYYY-MM-DD tabs, reusing `fetchIncomingOrders`) and reconcile pre-order state on the website: refresh `pre_order_incoming_us` (total upcoming qty) / `pre_order_date_us` (earliest upcoming date) on variants already live, and clear (policy=deny + delete the `_us` metafields) variants whose arrivals have all passed. **Enablement is decoupled from the sheet (2026-09-01):** a plain run never newly turns pre-order on — recording a production order's incoming inventory does not put products on pre-order. Turning it ON is the explicit step: `enable` prefixes (`--enable GAF`); `disable` (`'*'` = all live) force-clears despite upcoming arrivals, for pausing a pre-order. Scopable to a SKU prefix (e.g. "MPAD"); mirrors writes to `product_variants`. Batched per product via `productVariantsBulkUpdate`. CLI defaults to dry-run, `--send` to apply.
...
- **SKU size codes are `XL/2XL/3XL/4XL` — never `1X/2X/3X/4X`.** Legacy convention, consistent across every product; `1X/2X/3X` is display-only in the customer UI. Shopify variants must be created with XL-form SKUs (the Evey sports bra briefly launched with 1X-form and had to be renamed — every merchandising tool validates against the catalog, so a wrong-form catalog silently propagates everywhere). Supplier codes like `1X`/`2XT` map to XL-form via `skuCanonical`.
- **`product_cs_config.status` mirrors the Shopify product status automatically — no manual flip at launch.** The advisor only sees `active` config rows (`get_cs_product_config` filters), and `create_product` seeds `draft`; the reconcile (`customer-service/lib/csConfigStatus.js`) runs from the daily product sync, the products webhook (with in-process `initCsConfig` refresh), and `reload_products`, so launching a product activates its advisor config and archiving/reverting deactivates it. (Before this, a launched product stayed invisible to the advisor — the Evey launch, 2026-07.)
