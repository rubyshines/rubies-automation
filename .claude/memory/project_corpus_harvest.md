...
- Steps 4+5 code shipped (PR #97): `rebuildKnowledgeBase.js` (candidates → cs_knowledge_base with source_url/trust columns + Voyage embeddings, idempotent; --retire-legacy backs up + deletes the stale 63 articles) and weekly `refreshKb.js` wired as a Monday-gated daily-sync-all step (re-harvest, propagate amendments, flag drifted sources for re-extraction). PENDING: kb-rebuild-migration.sql in SQL Editor, then live rebuild + verify cs_get_knowledge + legacy retirement (then close the subsumed parked item). Step 6 (advisor search tool) stays gated until after 2026-07-23. Related ship: apologies fault-scoped in advisor prompt (PR #81 + pinned scenario), quantified by voice analysis (16/262 fit apologies in 2026 era).
