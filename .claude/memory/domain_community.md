...
- **Org onboarding is call-first, on Google Meet:** every new org inquiry gets a video call (Google Meet, Jamie sends the invite once a time is agreed) to walk through RUBIES and assess fit before anything else; only after the call do we send the partner survey link, and submissions then flow through `create_from_survey`. Partners are also offered direct purchase of gender-affirming clothing at the same country discount tiers as wholesale: 50% where shipments avoid tariffs (US; AU as long as packages stay under the 1000 AUD de minimis), 30% elsewhere (e.g. the Canadian and Swiss partners; UK confirmed 30% as well).
...
- **Proximity-tiered routing, load-balanced inside whichever tier fires:** local (same city or metro) first, then same state/province — but only when the nearest partner is already in it — then the closest 3 nationally. Each tier picks weighted-random by trailing-90-day item volume (from `donation_routings`, not the lifetime counter). Tiers added 2026-08 so a return goes to the org in the customer's own city instead of being spread across a region; the gate on the in-state tier is the load-bearing part, since an ungated "same state wins" would ship a far-northern-California return past Oregon down to Los Angeles. The weighted-random layer underneath replaced deterministic least-loaded-by-lifetime-count 2026-07: a newly added partner entered at count 0 and monopolized its region until it caught up (Montgomery blacked out Raleigh for two weeks), and counting routings instead of items undercounted big shipments. It now keeps partners sharing a metro or province active rather than balancing nationally — deliberately trading some spread for shorter shipping and items staying in the customer's own community. The lifetime `donations_routed` counter remains for impact reporting only. Single-item returns default to donate-locally, with two partner-routing overrides: the customer accepts the partner-info offer, or the CS advisor sets include_proof_ask on a refund-pattern-flagged refund (appends the locked donation proof ask; no ask on the no-partner local fallback).
