...
- **A takeover that only fires on collision never fires (2026-08-24).** The takeover above was reachable only from the duplicate-key branch — i.e. only if the same unit of work arrived a second time. At-least-once delivery does not mean *always* twice: once a webhook is ACKed nothing redelivers it, so the one case the takeover exists for (the process died, so nobody retried) is precisely the case that never triggers it. The recovery must ALSO be a **proactive sweep** on a timer, not just a branch inside the claim. Two things make the miss invisible while it lasts: the claim row is deliberately parked in a status that hides it from operator queues, and a detector that reports the gap without repairing it (our drift digest reports real misses and never auto-drafts, on purpose) reads as coverage while fixing nothing. Sweep interval is cheap; the failure is one customer never answered.
...
## UI wiring has no compiler — guard it with a static test

An inline `onclick="doThing()"` is a call into global scope. Nothing imports it, no bundler resolves it, and no test exercises it, so deleting the function leaves the button rendering perfectly and failing with `ReferenceError` only when a human clicks it.

**Why:** `sendComposedDraft` — the Send button for a message Jamie writes himself in the B2B panel — was deleted as collateral in an unrelated commit and shipped broken for twelve days. Every layer around it was fine: the button rendered, the `/compose` endpoint existed, `composeDraft` existed, the full suite passed. Only the one line joining them was gone. Worse than a dead button: composing is what *persists* the text, so a message typed into the panel lived only in the textarea, and the failed click plus a refresh destroyed it.

**How to apply:** `customer-service/test/dashboardHandlers.test.js` scans the dashboard's JS and HTML for inline handler attributes and asserts every name resolves to a definition. Keep it working when handler syntax changes; it costs nothing and covers the whole class. It carries a self-check asserting it still finds many handlers, because a regex that silently matches nothing stays green forever while testing zero things. The general pattern: **when a layer of wiring has no compile-time reference, add a cheap static assertion that it is still connected** — the same instinct as the calls-per-output tripwire for silent write failures.

**Server-side has the same hole: a lazily-destructured `require` is unchecked wiring (2026-08-27).** `const { getCompanyEmails } = require('./queueService')` inside a function body resolves at call time, not load time, so a name the target module defines and never exports is `undefined` in a file that parses, imports and tests clean. We use these deliberately (they break the queue/cadence/queueService cycles and keep the webhook server's startup cheap), so the fix is a static assertion, not a ban: `customer-service/test/lazyRequires.test.js` walks the lib directories and asserts every destructured internal binding is really exported — 1,017 bindings, and this was the only broken one. **What made it expensive is where it landed: inside a guard written to fail closed.** The B2B follow-up ladder's reply check catches its own errors on purpose (failing open would mean writing "just following up!" to someone who answered on Tuesday), so a TypeError became `could not resolve addresses to check` — worded, correctly, like Gmail being unreachable. Three UK follow-ups sat scheduled and were held on every 15-minute tick for a day, and the logs said a guard was doing its job. **A guard that can never pass is indistinguishable from a queue with nothing due**, so any fail-closed path needs its dependencies asserted somewhere that runs without it.

## Credentials are not permission — prove production, don't assume development

Local development runs with the production `.env` (it is symlinked into every worktree so a worktree can be tested at all). So a code path that writes *outside* the process — a Shopify discount code, a row the live CS routing reads, a theme publish that deploys the website — cannot tell a laptop from Railway by what it can reach. It has to ask.

**Why:** the Virtual Closet's "Shop with 20% off" button mints a real single-use Shopify code on every click, and approving a Pass-It-On centre creates a live `donation_partners` row and then publishes it to the theme, which auto-deploys rubyshines.com. Both run in the service, both had production credentials locally, and neither knew where it was. Four real discount codes were sitting ACTIVE in the store from test runs (found and revoked 2026-09-21); the donation-map path had not fired yet, purely by luck of the smoke script leaving Pass It On off.

**How to apply:**
- **Shape the check as "prove you are production."** `if (DEV_MODE) skip` fails open — an unmarked machine writes. `shared/liveWrites.js` keys on `RAILWAY_DEPLOYMENT_ID`, which Railway injects and `copy-railway-vars.js` refuses to copy, so an unknown environment refuses and the cost of being wrong is a skipped write.
- **Guard the automatic cascade, never the operator's hands.** The same publish reached by approving a centre is also an MCP tool Jamie runs by hand from a laptop as normal partner management. Guarding the shared helper at the bottom would have been "safer" and would have broken the real workflow. The dangerous thing is test data travelling, not a person acting deliberately — so the check goes at the call site that cascades, and the tool stays open. A test pins that asymmetry, because it reads like an oversight.
- **A guard stuck off is indistinguishable from nothing to do.** Same failure class as a fail-closed queue guard. Every service consulting it says which mode it booted in and reports `live` at `/health`, and the refusal logs one greppable line naming the write that did not happen. Without that, a renamed variable silently stops production writes forever while everything looks healthy.
- **Ask it of any new outward write:** if this ran on my laptop right now, what would appear in the world? If the answer is anything, it needs the guard.
