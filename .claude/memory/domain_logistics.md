...
- **The points-of-measure sketch on each QC tab is a repo asset, `customer-service/assets/qc-pom/<tech-pack handle>.png`,** cropped from the tech pack's Points of Measure page. The generator embeds it when present and says so when it is missing; a new product's first QC sheet needs its sketch added there.
