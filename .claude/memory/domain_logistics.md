...
- `scripts/fetchPomSketch.js`, `scripts/syncTechPackUrls.js`, `shared/googleDriveClient.js` — tech-pack registry and the points-of-measure sketch library behind the QC sheets.
...
- **QC tolerance is a house convention by size, applied in code, not read from the tech pack.** ±0.75 cm through size S, ±1.0 cm for M (16) to 1X, ±1.25 cm for 2X to 4X, tall variants by base size. The tech packs carry no tolerances and `tech_pack_specs.tolerance_cm` is the digitizer's default, so the sheet generator's `toleranceForSize` is the source; bands break where the tolerance steps up because the Tolerance column is shared across a band.
- **The points-of-measure sketch on each QC tab is a stored repo asset, `customer-service/assets/qc-pom/<tech-pack handle>.png|.jpg`, never drawn at generate time.** `scripts/fetchPomSketch.js` produces it: Drive exports the tech pack as .xlsx and the drawing is lifted off the Points of Measure sheet as the embedded image object it already is, chosen by its anchor (top band, right of the measurement table). Rendering the tab and cropping the drawing out of the page was tried first and abandoned — it has to guess where the drawing ends and the table begins, and that guess breaks on every layout variation. Line drawings stay PNG and photographic sketches stay JPEG, because re-encoding a photo as PNG multiplies its size; the generator reads either.
- **A tech pack states absolute targets per size, except the Ava bra, whose Graded Spec is relative to the size-10 sample** (a sample column plus per-step deltas). It was digitized into `tech_pack_specs` by walking the deltas, cross-checked against the Points of Measure tab's own final-measurement column, and an "Absolute Spec" tab was added to that tech pack stating the result. Where a garment's spec is capped by a component rather than the grade — Ava's cup height and width follow the available cup moulds above L — the component wins, because QC measures the garment that exists.
- **`tech_packs.tech_pack_url` is the registry of which Drive file is a product's tech pack,** matched by title and populated by `scripts/syncTechPackUrls.js`. The tech-pack folders are shared with the repo's service account, so Drive reads need no human export step. Every tech pack is a copy of one template, which is why the Points of Measure tab has the same gid in all of them.
