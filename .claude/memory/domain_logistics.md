...
- **The warehouse address is one string, stated by Jamie (2026-09-15), and the systems disagree with each other:** `Nitrologistics LLC, Attn: Rubies, 11470 SW AMU St, Tualatin OR 97062`. Use it for anything shipped to the warehouse (supplier consignments, samples, the mailing-address change) and as the return-to address. Warehance holds the same street under its ship-from and return address; the Shopify location record still shows the old 19870 SW 129th Ave building and our API token cannot edit it (`write_locations` scope missing), so it is fixed by hand in Shopify admin, not by code.
