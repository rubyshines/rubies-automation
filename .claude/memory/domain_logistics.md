...
- **Passport charges are billed through the Nitro/Warehance invoice — there is no separate Passport payment.** QBO has no Passport vendor and no passport-tagged transactions; Passport freight and customs/duties flow into Nitro's monthly bill (verified Jul 2026: the Feb 3 Nitro bill ≈ January's pick/pack/postage + Passport C&D to the dollar). Consequence for any cost analysis: never add `passport_invoices` totals on top of Nitro bill totals — that double-counts customs. `passport_invoices` is the per-shipment breakdown of a cost already inside the Nitro bills.
