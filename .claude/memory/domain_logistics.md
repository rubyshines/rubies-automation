...
- `customer-service/lib/tools/inboundReceiving.js` — 11 receiving/supplier MCP tools (`receive_shipment`, `update_inbound_shipment`, `reconcile_production_order`, `write_reconciliation`, `record_manual_order`, `amend_production_order`, `record_production_lots`, `upload_inbound_to_warehance`, `poll_inbound_receiving`).
...
- **A Warehance inbound shipment (ASN) is header-mutable only, and there is one per physical consignment.** `PATCH /inbound-shipments/{id}` covers reference number, dates, tracking, status, closed and notes — **line items cannot be changed after creation, and there is no DELETE endpoint**. Correcting *what is on* an ASN therefore means posting a replacement and asking the warehouse to disregard the old one, so the consignment split has to be right before upload (and a duplicate POST leaves a phantom ASN the warehouse can still receive against). An order routinely arrives in more than one consignment — ocean container plus a courier parcel, or a held batch following later — and each gets its own `inbound_shipments` row and its own ASN. Transfer numbers run `<code>`, `<code>-2`, …; the first shipment keeps the bare production code because its reference is already live in Warehance. **No shipment is recorded without an expected arrival date** — Warehance renders a missing one as `0001-01-01`, which the warehouse cannot plan receiving around and which reads as a real date in both systems. When it isn't given it's estimated from the carrier's mode (`transitEstimate.js`); when the mode isn't recognisable `receive_shipment` writes nothing and asks how long the shipment is in transit. Ask rather than guess, and prefer a visible estimate the operator corrects over a silent blank.
