...
- **The points-of-measure sketch on each QC tab is a repo asset, `customer-service/assets/qc-pom/<tech-pack handle>.png`,** cropped from the tech pack's Points of Measure page (Drive → File → Download → PDF, then crop the lettered sketch at 300 dpi). The generator embeds it when present and says so when it is missing; a new product's first QC sheet needs its sketch added there. The claude.ai Drive connector can only export tech packs under about 7 MB; larger ones (Sassy, Shorty, Ruby, Sky) have to be downloaded by hand.
