...
- **Onboarding form now asks the three questions (2026-09-11):** how people get items (self-classifies the programme profile at intake, no reading step), whether the org buys gear (a budget signal the advisor is given), and whether they'd want an affiliate programme (recorded for a list; the programme does not exist and is never offered). Answers land on the company row on a confirmed ingest.
- **Programme profiles (2026-09-11):** what each org actually runs is now on the record and in front of you before a call — standing closet, gear by request, periodic events, or nothing they hold themselves. 84 orgs read from their own words (survey write-ups, call notes, their replies): 23 standing closets, 19 by request, 6 events, 8 with no programme of their own, 27 whose entire history says nothing about it. 111 orgs have no evidence at all, which reads as "not read" rather than blank. The distinction matters commercially: a closet takes volume, a by-request programme takes codes, and an org with no programme is a referral route, not a donation partner.
