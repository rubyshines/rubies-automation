...
**Discount Management (MCP):** `manage_discount` tool + `managed_discounts` Supabase registry — volume discounts + sales lifecycle (start/extend/end, optional attached free gift), with audit/reconcile and an active-sales banner in the daily order alerts. Replaced the rubies-utilities discount script + Google Sheet. Sales self-close: a daily-sync sweep (`sweepExpiredSales`) runs the full end_sale cleanup (registry status, theme banner metafields, attached gift) for any sale past its end date — Shopify expires the discount nodes on its own, so nothing else forced the rest (added 2026-09-01 after the Pride Sale sat "active" for two months with its gift offer still enabled).
