...
| **virtual-closet** | `node virtual-closet/server.js` | `railway/virtual-closet.toml` (start command and health check are also set on the service, since Railway's API no longer accepts a config file path) | The Virtual Closet web service, currently at the Railway domain `virtual-closet-production-5862.up.railway.app` until a subdomain is pointed at it. Needs `VC_BASE_URL` (its public URL) and `VC_OPS_BASE_URL` (the dashboard); both are set on the main service and copied. Auto-deploys on push to main. |
