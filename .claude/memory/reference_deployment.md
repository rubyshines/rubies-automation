...
|---------|------------|--------|-------|
| **rubies-automation** (webhook server, main) | `node webhooks/server.js` | root `railway.toml` | Express server on Railway-assigned port. Receives Shopify, Gorgias, Gmail push webhooks. It is also where work that has to tick all day lives, rather than a cron service — seven sweeps: the warehouse-hold backstop (`reconcilePendingHolds`, 3 min), unnotified pre-order outreach (`sweepUnnotifiedPreOrders`, 10 min, seeds customer drafts), the Gorgias spam-gate fast path (`runSpamGate`, 15 min over tickets updated since the last tick — Gorgias sends no webhook for spam-flagged customer messages), the B2B follow-up send pass (`runSendPass`, 15 min, where the unattended-send guards run), the de-allocation watch (`sweepDeallocations`, hourly), stranded intake claims (`sweepStaleDraftClaims`, 5 min), and post-call meeting notes (`runPostCallSweep`, 15 min, for a call that ended in the last six hours; the nightly `Meeting Notes` step is its backstop). Env-var source of truth. Auto-deploys on push to main. |
...
| **virtual-closet** | `node virtual-closet/server.js` | `railway/virtual-closet.toml` (start command and health check are also set on the service, since Railway's API no longer accepts a config file path) | The Virtual Closet web service. Public address is **closet.rubyshines.com** (custom domain on the service, target port 8080, CNAME target `x1ryzei1.up.railway.app`); the Railway domain `virtual-closet-production-5862.up.railway.app` keeps working alongside. `VC_BASE_URL=https://closet.rubyshines.com` is set on rubies-automation, virtual-closet and cs-dashboard (2026-09-21) and is what every link, QR and sign carries. `VC_OPS_BASE_URL` is the dashboard. Auto-deploys on push to main. |
...
KB Refresh runs as a `daily-sync-all` step self-gated to Mondays UTC (re-harvest kb_sources, propagate kb_candidates into cs_knowledge_base with embeddings, flag drifted sources needing re-extraction) — see `customer-service/sync/refreshKb.js`. Free swimwear (import new applications, then reconcile register/order/expire/resend) runs as two sub-pipelines of `daily-sync-all` (`Free Swimwear Apps` + `Free Swimwear Lifecycle`), not a separate cron service. `Virtual Closet` (after those) reconciles the box ledger from the order mirror, sends by-hand and pickup reminders, monthly statements on the 1st, and the operator's needs-attention digest. `Wholesale Pricing Page` republishes `assets/wholesale-pricing.json` on the theme after the Products sync (no-op unless the data changed; needs `GITHUB_TOKEN`). `Meeting Notes` (after Calendar Meetings, before Relationship Summaries) fetches recent calls' Wispr Flow recordings onto `b2b_meetings` and lifts their action items onto `b2b_commitments`; it skips itself with a warning when Wispr is not connected. The Wispr OAuth token lives in the Supabase `oauth_tokens` row (key `wispr`), minted once by `node scripts/authWispr.js`; `WISPR_TOKEN_JSON` is an optional bootstrap env var, not required.
...
| Virtual Closet (dev) | 3850 | `PORT=3850 VC_EMAIL_MODE=console node virtual-closet/server.js` — emails print to the console with their links when SendGrid is off or `VC_EMAIL_MODE=console`. `node virtual-closet/scripts/seedDemo.js` seeds a demo centre; `node virtual-closet/scripts/smoke.js` walks the whole flow. | To preview a branch on a phone, claim an `ra-N.ngrok.app` domain from the `/preview` pool and run `PORT=<free> VC_BASE_URL=https://ra-N.ngrok.app node virtual-closet/server.js` (record the pids in `/tmp/preview-<port>.pids` as `/preview` does); the page and emails then link to the tunnel. Off Railway the Shop button sends shoppers to the store without minting a code and the donation-map publish is skipped (`shared/liveWrites.js`), so money paths are only testable on the deployed service or by writing fake orders through `ledger.recordOrder`.
...
- **Propagation:** `node scripts/copy-railway-vars.js` reads all vars from the main service via Railway GraphQL API and copies them to the services in its `CRON_SERVICES` array — currently 12 (the 10 crons above + cs-dashboard + virtual-closet). Skips Railway-injected vars (`RAILWAY_*`, `NIXPACKS*`, etc.).
- **When to run:** After adding or changing any env var on the main service.
- **Setting a variable (2026-09-21):** the Railway GraphQL token in `~/.railway/config.json` now answers "Not Authorized" to every mutation (variables, domains), while reads still work. Use the CLI instead: `railway link --project ac9b7c78-8b5d-406a-929f-833fec78cfa9 --environment production --service <name>` once in the worktree, then `railway variable set KEY=VALUE --service <name>` per service (names: rubies-automation, virtual-closet, cs-dashboard, the crons). Railway redeploys the service on a variable change unless `--skip-deploys`. Custom domains need the Railway dashboard.
- **closet.rubyshines.com (2026-09-21, pending):** the CNAME at domain.com was changed three times as Railway re-assigned targets; their nameservers kept round-robining all three values, so the certificate stayed at "validating ownership". Jamie chose to wait 12 hours. Check: `dig +short @ns1.domain.com closet.rubyshines.com CNAME` must return only `x1ryzei1.up.railway.app`, then `curl https://closet.rubyshines.com/health` returns 200. If it never settles, the panel probably holds more than one `closet` row; delete them all, confirm both nameservers return nothing, add one back.
...
- **`RAILWAY_DEPLOYMENT_ID` is the "am I production?" signal** (`shared/liveWrites.js`). Railway injects it into every deployment and `copy-railway-vars.js` skips the `RAILWAY_` prefix, so it can never reach a local `.env` — which is what makes it trustworthy. Service code that writes *outside* itself consults it and refuses when it is absent, since local dev shares the production `.env` and credentials alone are not permission. Deliberately not applied to MCP tools: an operator running one by hand from a laptop is doing their job. **If a guarded write ever stops happening in production, check this first** — `curl <service>/health` reports `live: true|false`, and the Virtual Closet logs which mode it booted in. A guard stuck off looks exactly like nothing needing to be written.
