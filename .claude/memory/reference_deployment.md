...
| Virtual Closet (dev) | 3850 | `PORT=3850 VC_EMAIL_MODE=console node virtual-closet/server.js` — emails print to the console with their links when SendGrid is off or `VC_EMAIL_MODE=console`. `node virtual-closet/scripts/seedDemo.js` seeds a demo centre; `node virtual-closet/scripts/smoke.js` walks the whole flow. |
