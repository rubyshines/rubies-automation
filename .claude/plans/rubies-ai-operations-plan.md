...
**Night-2 progress (2026-06-11, overnight loop): steps 1-6 BUILT + committed on
sprint/outreach (commits 1834416, 10b29be, a0a6c23, 5989d46). 812/812 tests.**
- Step 7 (seed run) BLOCKED on two things: (a) JAMIE: paste
  gmail-management/b2b-outreach-schema.sql into the Supabase SQL Editor; (b) Opus 529
  congestion clearing (advisors run on Opus). Then: `node b2b-outreach/sweep.js --draft 5`
  from the outreach worktree produces the first real drafts.
- Step 8 (variant plumbing): columns + flow-through DONE in 1-6; variant selection logic
  comes with template variants later.
- Still to do before cowork merge: register sendB2bEmail + a b2b_queue tool in the operator
  console (customer-service/lib/operatorTools.js + server.js, canonical donationPartners
  pattern); steer scenario still 529-blocked (advisor-api branch waits on it).

- [x] 1. Schemas: extend gmail-management/b2b-schema.sql (or new b2b-outreach-schema.sql):
...
- [x] 2. lib: outreach/cadence.js — pure due-condition evaluators per Trigger-3 table +
      next_action_date writer (unit tests per type).
- [x] 3. send tool: customer-service/lib/tools/sendB2bEmail.js — two-phase per Design #6
...
- [x] 4. Reply correlation: extend Gmail Pub/Sub path — sender ∈ b2b_contacts/general_email
      ...
      technical rules.
- [x] 5. Queue: lib/outreach/queue.js — 6-tier computation per locked decision #6; daily 6am
...
- [x] 6. Advisors: wire b2b_sales_advisor + b2b_community_advisor from the prompts draft
...
- [x] 8. Variant tracking: variant_id on drafts/messages (locked #15) — plumbing only, A/B
      analysis later.
