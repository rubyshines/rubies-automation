 ...
 *   clear_due → next_action_date=null (nothing to send now; the cadence decides
 *                                      when it comes back — the Tier-5 exit)
 *
...
function computeTriage(action, { reason = null, until = null, now = new Date(), source = 'operator', note = null, restoreTo = null } = {}) {
  switch (action) {
    ...
    case 'clear_due':
      ...
      // hands the company back to the cadence, which will surface it again on
      // a real trigger (a reply, an order, the October check-in). Deliberately
      // touches nothing else: not a deferral, not a vetting decision, and the
