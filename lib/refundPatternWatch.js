...
async function computeRefundPatternReport({ days = 7 } = {}) {
  ...
  const drafts = await fetchAllPaginated(() =>
    sb.from('cs_ai_drafts')
      .select("id, gorgias_ticket_id, message_type, created_at, flags:structured_output->prescription->flags, adv_status:structured_output->>status, routing_reason:structured_output->>routing_reason")
