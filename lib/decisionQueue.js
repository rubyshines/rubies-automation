 ...
 *  - Passport customs import overdue (newest invoice_date older than a
 *    fortnight). The only cost pipeline run by hand, so the only one that can
 *    silently stop; five un-imported invoices went unnoticed for a month in
 *    Aug 2026. See finance/lib/passportImportFreshness.js.
 *
 * All reads fail-soft: a missing table produces no items, never an error.
 */
const { getSupabaseClient, fetchAllPaginated } = require('../shared/supabaseClient');
const { collectPassportFreshnessItems } = require('../finance/lib/passportImportFreshness');
...
async function collectDecisions() {
  ...
  const settled = await Promise.allSettled([
    ...
    collectPassportFreshnessItems(sb),
  ]);
