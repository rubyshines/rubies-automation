...
async function ledgerTotal(startISO, endISO) {
  ...
  const rows = await fetchAllPaginated(() => sb.from('ai_calls')
    .select('cost_usd')
    .gte('created_at', startISO)
    .lt('created_at', endISO)
    .order('id', { ascending: true }));
  return rows.reduce((total, r) => total + Number(r.cost_usd || 0), 0);
}
...
async function run() {
  if (new Date().getUTCDate() !== 1) {
    // Monthly task — skipping on the other 30 days is normal, not a failure.
    return { sources: { bill_reconcile: { skipped: true, detail: 'runs on the 1st of the month' } }, status: 'ok' };
  }
  try {
    const verdict = await reconcilePreviousMonth();
    return { sources: { bill_reconcile: verdict }, status: 'ok' };
  } catch (e) {
    // Bill reconciliation failing is a real finance alarm — surface as a failure.
    return { sources: { bill_reconcile: { failed: true, error: e.message, alarm: true } }, status: 'failure' };
  }
}
