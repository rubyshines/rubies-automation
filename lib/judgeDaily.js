...
/**
 * `tool_gap` ships with a migration applied by hand in the Supabase SQL editor,
 * so the runtime has to tolerate arriving first: naming the column in an insert
 * before it exists would fail EVERY judge write, not just the new field, and
 * take the nightly run down with it. Same hazard the system_flags reader hit —
 * a runtime deployed ahead of its migration must degrade, never throw.
 * null = not yet probed, true/false = known.
 */
let _hasToolGapColumn = null;

function isMissingToolGapColumn(error) {
  if (!error) return false;
  // PGRST204: column not found in schema cache. 42703: undefined_column.
  return error.code === 'PGRST204' || error.code === '42703'
    || /tool_gap/i.test(error.message || '');
}

/** Insert a verdict, dropping tool_gap if the column isn't there yet. */
async function insertJudgment(sb, base, toolGap) {
  if (_hasToolGapColumn === false) {
    return sb.from('cs_draft_judgments').insert(base);
  }
  const res = await sb.from('cs_draft_judgments').insert({ ...base, tool_gap: toolGap || null });
  if (res.error && isMissingToolGapColumn(res.error)) {
    _hasToolGapColumn = false;
    console.warn('[judgeDaily] tool_gap column absent — run migrations-2026-08-11-judge-tool-gap.sql; storing verdicts without it');
    return sb.from('cs_draft_judgments').insert(base);
  }
  if (!res.error) _hasToolGapColumn = true;
  return res;
}
...
async function judgeRecentDrafts({ days = 3, cap = DAILY_CAP } = {}) {
  ...
  let judged = 0, failed = 0, factsQueued = 0, toolGaps = 0;
  for (const d of pending) {
    try {
      ...
      const { error } = await insertJudgment(sb, {
        draft_id: d.id,
  ...
  return { judged, failed, facts_queued: factsQueued, tool_gaps: toolGaps, pending_beyond_cap: Math.max(0, drafts.length - done.size - cap) };
}
...
async function computeJudgeMetrics({ days = 30 } = {}) {
  const sb = getSupabaseClient();
  const since = new Date(Date.now() - days * 86400000).toISOString();
  const BASE_COLS = 'category, severity, draft_may_be_right, message_type, rationale, draft_id, created_at';
  const read = cols => fetchAllPaginated(() =>
    sb.from('cs_draft_judgments')
      .select(cols)
      .gte('created_at', since)
      .order('created_at', { ascending: false })
  );
  // Same pre-migration tolerance as the insert path: naming a column that does
  // not exist yet would throw and drop the whole quality line from the digest.
  let rows;
  if (_hasToolGapColumn === false) {
    rows = await read(BASE_COLS);
  } else {
    try {
      rows = await read(`${BASE_COLS}, tool_gap`);
    } catch (e) {
      if (!/tool_gap/i.test(e.message || '')) throw e;
      _hasToolGapColumn = false;
      rows = await read(BASE_COLS);
    }
  }
  ...
  // Corrections a tool should have answered. These are data/tool tickets, not
  // operator facts — the whole point of splitting the signal out.
  const toolGaps = rows.filter(r => r.tool_gap).map(r => ({
    draft_id: r.draft_id, message_type: r.message_type, tool_gap: r.tool_gap,
  }));
  return {
    window_days: days, ...summary,
    draft_may_be_right: flagged.slice(0, 10),
    high_severity: highSeverity,
    tool_gaps: toolGaps.slice(0, 10),
    tool_gap_count: toolGaps.length,
  };
}
...
module.exports = { run, judgeRecentDrafts, computeJudgeMetrics, summarizeVerdicts, normalizeFact, factSimilarity, FACT_DUPE_THRESHOLD,
  // exported for tests: the pre-migration tolerance around the tool_gap column
  isMissingToolGapColumn, insertJudgment };
