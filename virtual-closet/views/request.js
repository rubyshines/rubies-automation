...
function itemRow(products, centre, idx, values = {}) {
  ...
  const styleOpts = products.map(p => `<option value="${p.key}" ${values.style === p.key ? 'selected' : ''} data-colours="${esc(p.colours.join('|'))}" data-sizes="${esc(sizesFor(p.key, centre).join('|'))}">${esc(p.title)}</option>`).join('');
