...
function requestsTable({ centre, requests, role, byHand, filter = 'all' }) {
  ...
  const rows = shown.map(r => {
    ...
    return `<tr><td data-label="Name">${esc(r.name)}</td><td data-label="Items">${itemsHtml(r.items)}</td><td data-label="Pickup or ship">${r.delivery === 'ship' ? 'Ship <span class="fine">($15 from the box)</span>' : 'Pickup'}</td><td data-label="Their words">${wordsCell(r, role)}</td><td class="soft" data-label="Requested">${ago(r.created_at)}</td><td data-label="">${answer}</td></tr>`;
  }).join('');
  ...
  return `${chips}<div class="tbl"><table class="stack"><thead><tr><th>Name they go by</th><th>Items</th><th>Pickup or ship</th><th>Their words</th><th>Requested</th><th>${byHand ? 'Answer' : 'Status'}</th></tr></thead><tbody>${rows || '<tr><td colspan="6" class="soft">No requests yet.</td></tr>'}</tbody></table></div>`;
}
...
<section class="two">
  ...
  <div>
    ...
    <div class="card"><h3>This month</h3><div class="kpis"><div class="kpi"><b>${month.visits}</b><span>Link visit${month.visits === 1 ? '' : 's'}</span></div><div class="kpi"><b>${month.orders}</b><span>Order${month.orders === 1 ? '' : 's'}</span></div><div class="kpi"><b>${month.sponsors}</b><span>Sponsor${month.sponsors === 1 ? '' : 's'}</span></div><div class="kpi"><b>${month.requests}</b><span>Request${month.requests === 1 ? '' : 's'}</span></div></div></div>
  </div>
</section>
...
function history({ centre, user, boxes, donations, statements, actingAs }) {
  const body = `
<section><h2>Boxes</h2><div class="tbl"><table class="stack stack-2up"><thead><tr><th>Box</th><th>Sent</th><th>Raised</th><th>RUBIES match</th><th>Items</th><th>Requests filled</th><th>Tracking</th></tr></thead><tbody>
${boxes.map(b => `<tr><td data-label="Box"><a href="/history/box/${b.number}">#${b.number}</a></td><td data-label="Sent">${b.status === 'open' ? '<span class="pill">open</span>' : fmtDate(b.sent_at)}</td><td data-label="Raised">${dollars(b.raised)}</td><td data-label="RUBIES match">${b.status === 'open' ? '<span class="soft">at shipping</span>' : dollars(b.match)}</td><td data-label="Items">${b.items_count ?? '…'}</td><td data-label="Requests filled">${b.requestsFilled}${b.waiting ? ` + ${b.waiting} waiting` : ''}</td><td class="soft" data-label="Tracking">${b.carrier ? `${esc(b.carrier)} ${esc(b.tracking_number || '')}` : '—'}</td></tr>`).join('')}
