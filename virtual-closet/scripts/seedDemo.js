...
async function main() {
  ...
  if (!box) {
    ...
    must(await db().from('vc_ledger').insert([
      ...
      { centre_id: centre.id, box_id: b1.id, kind: 'match', amount_cents: 31200, source_type: 'box', source_id: `match-${b1.id}` },
    ]), 'b1 ledger');
    ...
    must(await db().from('vc_ledger').insert([
      ...
      { centre_id: centre.id, box_id: box.id, kind: 'sponsor', amount_cents: 5000, source_type: 'seed', source_id: 'b2-s1' },
      { centre_id: centre.id, box_id: box.id, kind: 'sponsor', amount_cents: 1600, source_type: 'seed', source_id: 'b2-s2' },
      { centre_id: centre.id, box_id: box.id, kind: 'sponsor', amount_cents: 1000, source_type: 'seed', source_id: 'b2-s3' },
      { centre_id: centre.id, box_id: box.id, kind: 'centre_add', amount_cents: 1600, source_type: 'seed', source_id: 'b2-c1' },
    ]), 'b2 ledger');
    must(await db().from('vc_requests').insert([
      ...
      { centre_id: centre.id, box_id: box.id, email: 'mel@example.com', name: 'Mel', items: [{ style: 'sassy', colour: 'Mint', size: 'M' }], delivery: 'ship', address: { street: '9 Elm St', city: 'Urbana', region: 'IL', postal: '61801' }, status: 'approved', verified_at: daysAgo(5), decided_by: 'system', decided_at: daysAgo(5), created_at: daysAgo(5) },
