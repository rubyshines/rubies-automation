...
- **Memory — read first:** MEMORY.md is auto-loaded every session. At session start: (1) ALWAYS read `feedback_collaboration.md` and `feedback_technical_rules.md` (the "Always Read" section), then (2) read topic memories relevant to the user's request. Memory is the map; only go to code for specifics memory doesn't cover. Never launch exploration agents for things already documented in memory.
- **Memory — write carefully:** Feedback ONLY goes in `feedback_collaboration.md` (how we work) or `feedback_technical_rules.md` (how we build). Domain-specific rules go in the relevant topic file. NEVER create new standalone feedback files. Topic files use prefixes: `project_*.md`, `domain_*.md`, `user_*.md`, `reference_*.md`. Check MEMORY.md index before creating anything new.
- **Memory — keep organized:** MEMORY.md is a pure index — one-line entries only, under 200 lines, no inline content. When working in a domain and you notice stale memory, update it. Never let memory drift from reality.
