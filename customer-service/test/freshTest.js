/**
 * Fresh Test — Run hybrid advisor on 20 RANDOM training conversations (NOT holdout)
 * to verify improvements generalize and we haven't overfit to the holdout set.
 *
 * Usage: node customer-service/test/freshTest.js [count] [seed]
 */
...
async function main() {
  ...
  // Training set = first 160 entries (holdout is 160-199)
  const training = dataset.slice(0, 160);

  // Shuffle with seeded random and pick COUNT
  const rng = seededRandom(SEED);
  const indices = training.map((_, i) => i).sort(() => rng() - 0.5);
  const selected = indices.slice(0, COUNT).map(i => training[i]);

  console.log(`=== Fresh Test: ${selected.length} conversations (seed=${SEED}) ===`);
  console.log(`Tickets: ${selected.map(c => c.ticket).join(', ')}\n`);
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    console.log(`[${i + 1}/${selected.length}] Ticket ${ticket} (${conv.wordCount}w, created=${conv.createdOrder}, donation=${conv.mentionedDonation})`);
    ...
    try {
      ...
      if (jamieIdx >= 0) {
        ...
        const gorgiasContext = contextParts.join('\n\n');
    ...
    // Save progress
    fs.writeFileSync(
      path.join(__dirname, 'fresh-results.json'),
  ...
  console.log('\n========================================');
  console.log('         FRESH TEST RESULTS');
  console.log('========================================');
  ...
  console.log(`\nResults saved: customer-service/test/fresh-results.json`);
}
