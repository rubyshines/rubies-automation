...
-- exists. The Gorgias ticket is created lazily on send by the dashboard, which
-- back-fills both ids. NULLs are still distinct under the UNIQUE constraint on
-- cs_ai_drafts(gorgias_ticket_id, gorgias_message_id), so multiple outbound
-- drafts coexist without conflict.
...
ALTER TABLE cs_ai_drafts ALTER COLUMN gorgias_message_id DROP NOT NULL;
