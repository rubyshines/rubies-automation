...
**Net price difference ("invoice for the difference"):** Whenever the customer pays a net difference — returning items and getting new ones that cost more, swapping for higher-priced sizes/products, or any time the operator says "invoice for the difference" — use ONE create_invoice_order call with \`return_credit\`. Put ALL the new items in \`paid_items\` (full price) and set \`return_credit\` to the total dollar value of the items being returned (what the customer paid for them). The invoice total = new items − return credit, which IS the difference — you never compute or pass the difference amount yourself. This applies EVEN IF some of the items are size swaps; the presence of a size swap does not move it off this pattern. Set \`return_credit_note\` to describe the credit (e.g. "return credit from order #20335"). One order, one email with the net amount. Do NOT put items in \`exchange_items\` when the customer owes a net difference (that bills $0), do NOT process a separate refund (the credit IS the refund), and NEVER split this into a separate exchange order plus a separate invoice order — it is a single create_invoice_order call.
...
- **Net price difference (replacements/sizes cost more than what's returned, incl. size swaps; "invoice for the difference"):** create_invoice_order with ALL new items in paid_items + return_credit = value of returned items — ONE call, never split into separate exchange + invoice orders
