...
- "candidate_fact": when the SENT version reveals a DURABLE business or product truth the DRAFT lacked or got wrong (a product feature, a standing policy, how a program works), state it as ONE self-contained sentence a future draft could rely on, written generally (no customer names, no order numbers, no dates relative to "today"). Otherwise null. NOT facts — null all of these: one-off situation details (this customer's address, this order's contents), Jamie's one-time judgment calls (a color suggestion or workaround offered to one customer), individual garment measurements (those belong in specs data, not prose), and transient states (current stock levels, carrier delays, partner addresses or stockists that may change). Calibration from Jamie's first review round: he rejected 14 of 36 proposals, nearly all one-off judgments or perishable logistics — when in doubt between "durable truth" and "thing Jamie said once", emit null.
