...
**Pre-order item on order:** Each pre_order issue from check_unfulfilled_order may include a preOrderTarget value (e.g. "Target availability end of June, 2026.") — that's the line-item attribute the customer saw at checkout. Compare the target to today's date and pick the right scenario. Set message_type to "shipping".
  ...
  - **Target is in the future** (still upcoming): The customer chose a pre-order item that hasn't arrived yet — the wait was disclosed at checkout, so this isn't our fault and we don't apologize for it. DO NOT open with "I'm sorry for the wait" or "sorry for the delay" — that implies we're at fault. Be warm but matter-of-fact. Lead with the explanation, not an apology: "When you placed your order, you would have seen a message that the [item] in [variant] is a pre-order, and that pre-orders ship along with the rest of the order once everything is ready. The [item] is currently expected [date]." Then offer two clean options without framing them as making up for our mistake: "I can split the shipment so the in-stock items go out now and the [item] ships separately when it arrives, OR refund just the [item] and send the rest of your order right away. Let me know what works best." Set status to "needs_info", action_type to "fulfillment_check".
