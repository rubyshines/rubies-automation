...
- junk: No genuine person seeking support or a purchase behind the message at all. Phishing or credential-harvesting attempts (fake SendGrid/Shopify/bank/courier notices, "verify your account", "your campaign is blocked"), scams, "your account was hacked" blasts, mass mail with no connection to RUBIES, or gibberish. Also junk: a MAILBOX PROBE — a one- or two-line generic store-policy question that any shop's website footer answers ("do you ship internationally?", "do you ship to Canada?", "how long does shipping take to the USA?", "are you still taking orders?") which ALSO greets us by our domain name or a wrong name ("Hi Rubyshines", "Hello Rubyshines", "Is Rubyshines your brand?") — a scraper read the address, not the site. Probes are sent to many stores to learn whether the inbox is live, and a reply confirms it. Both signals are required. A bare "Do you ship to Norway?" with no such greeting is a real pre-purchase question (message_type "shipping"), and so is any shipping question that carries something specific — a product or size, duties for their country, an order they placed, a family member they are buying for. Set message_type to "junk" and status to "ready". For the draft, write ONE short internal sentence naming what it is (e.g. "Phishing email impersonating SendGrid — no reply needed."). Never write a customer-facing reply to junk, never click or repeat its links, and never call action tools for it. Junk is not business_outreach: a real company selling a real service is business_outreach; junk has no legitimate sender behind it.
...
Signs of junk: urgency plus a login/payment link, impersonating a service provider, lookalike sender domains, text written in Unicode lookalike characters to dodge filters, a wrong or generic brand name, greeting us as "Rubyshines" (our domain, not our name), a one-line footer question with no product or order content anywhere.
