...
async function handleDraft({ supplier, review_tab }) {
  ...
  const md = [
    ...
    '',
    `Built order tab **"${r.tabName}"** (clean: SKU + Qty) in the 2026 Production Numbers sheet from **"${r.sourceTab}"** — **${r.skuCount}** SKUs · **${r.totalUnits.toLocaleString()}** units.`,
    r.pricing ? `💰 Pricing companion tab **"${r.pricingTabName}"** — landed **$${Math.round(r.pricing.landed).toLocaleString()}** (COGS $${Math.round(r.pricing.cogs).toLocaleString()} · shipping $${Math.round(r.pricing.freight).toLocaleString()} · taxes $${Math.round(r.pricing.duty).toLocaleString()}). Removed automatically when you submit.${r.pricing.missing && r.pricing.missing.length ? ` ⚠️ no cost on file for: ${r.pricing.missing.join(', ')}` : ''}` : '',
    warnBlock(r.warnings),
    '',
...
module.exports = [
  ...
  },
  {
    ...
    description: 'Stage 2: read the founder-edited Qty to Order from a projections review tab (from draft_order_review) and write TWO tabs to the 2026 Production Numbers Google Sheet: a clean order tab (SKU + Qty), named "<date> - <supplier> <descriptor>" (e.g. "2026-06-29 - Kali Swim and Underwear"), and a companion "... (Pricing)" tab with the cost estimate (per-unit + extended COGS / shipping / taxes / landed from current product_costs). Dedupes duplicate SKU rows and warns on sub-floor / off-step edits. Jamie eyeballs it, then calls submit_production_order with the order tab name; submit removes the pricing tab.',
    inputSchema: {
      type: 'object',
      properties: {
