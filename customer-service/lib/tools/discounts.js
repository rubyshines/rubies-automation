...
const tools = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        free_gift_minimum: { type: 'number', description: 'start_sale / update_sale: minimum cart subtotal in USD to qualify for the attached free gift. REQUIRED on start_sale whenever free_gift_handle is set — pass 0 explicitly for "every order", or a threshold. If a gift is attached without this, the tool stops and asks. On update_sale it is optional (defaults to the sale\'s stored/live gift minimum).' },
        free_gift_text: { type: 'string', description: 'start_sale / update_sale: short customer-facing description of the free gift, woven into the sale copy (banner, modal, tag, announcement). e.g. "free Pride merch", "a free Pride bracelet". The threshold is appended automatically as "$<min> USD+" (or "with every order" when the minimum is 0).' },
      },
      required: ['action'],
    },
    handler: async (input) => {
      ...
      try {
        switch (input.action) {
          ...
          case 'start_sale':
            ...
            await startSale({ name: input.name, tiers: input.tiers, collectionHandle: input.collection_handle, startDate: input.start_date, endDate: input.end_date, freeGiftHandle: input.free_gift_handle, freeGiftMinimum: input.free_gift_minimum, freeGiftText: input.free_gift_text }, log);
            await appendAdminLinks(input.name, log);
            break;
          case 'update_sale':
            needName('update_sale');
            if (!input.free_gift_text) throw new Error('update_sale requires free_gift_text (the gift description to weave into the sale copy).');
            await updateSaleGift(input.name, { freeGiftText: input.free_gift_text, freeGiftMinimum: input.free_gift_minimum }, log);
            break;
          case 'extend_sale':
