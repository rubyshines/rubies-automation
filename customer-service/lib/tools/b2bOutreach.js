...
module.exports = [
  {
    name: 'b2b_template',
    description: "Operator message templates for B2B continuations — predefined bodies with every fill deterministic (contact first name, org name, country discount, meeting day, survey link), never a model call. With only company_id: list which templates apply and what each would include. With template_id: fill it and store it as the company's pending compose draft (advisor null — a human owns the words), attachments included, ready to edit in the Outreach panel or send via send_b2b_email. 'setup_call' answers an interested org or retailer and asks for 30 minutes, letting THEM suggest times (it includes the program summary only when we never sent them an intro). 'partner_onboarding' (orgs only) is the post-call email: agreement PDF attached (rendered fresh at send), onboarding survey link, and a [NOTES FROM THE CALL...] placeholder the operator must replace — the send path refuses a body still carrying it. 'missed_call' is the reschedule ask after a recorded no-show. 'waiting_in_room' is offered only while a booked call is in progress (from 5 minutes before its start until its end, no outcome recorded): a fresh email under the subject 'In the meeting room right now' saying Jamie is in the room and asking if they still plan to attend, with the event's Meet link — for the contact who has not joined. 'wholesale_terms' (retailers only) sends the wholesale terms inline at the company's rate (stored wholesale_discount_percent, else US/AU 50% and elsewhere 30%) with a link to rubyshines.com/pages/wholesale-pricing carrying their country (?country=ca shows the 30% view); no attachment — the page is the price list.",
    inputSchema: {
      type: 'object',
      properties: {
        company_id: { type: 'string', description: 'b2b_companies id.' },
        template_id: { type: 'string', description: "Omit to list. 'setup_call' | 'partner_onboarding' | 'missed_call' | 'waiting_in_room' | 'wholesale_terms'." },
      },
      required: ['company_id'],
    },
