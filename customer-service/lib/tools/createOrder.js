...
const tools = [
  {
    name: 'create_order',
    description: 'Create a new order for any customer (new or existing). Supports paid orders, free orders (samples/gifts), and custom discounts. Two-phase: create_order always stages a DRAFT and returns its admin link + Shopify-calculated totals; review it, then call create_order_complete with the draft_order_id to finish (free orders are marked paid; paid orders get the invoice sent). Do NOT re-call create_order to confirm — that just creates a second draft. For new customers, provide email + name + address and the customer is created in Shopify automatically. ONE-BOX MERGE: when in-stock items from another UNFULFILLED order should ship together with this new order (e.g. a free replacement plus the shippable part of a pending pre-order split, so only ONE package leaves the warehouse), include those items in THIS order (free=true — they were already paid on the other order), complete it, then call split_shipment on the other order with ship_with_order=<this new order number> to placeholder-fulfill them there so the warehouse cannot double-ship.',
    inputSchema: {
      type: 'object',
      properties: {
        email: {
