...
(function () {
  ...
  // A link-mode centre's figures are in its own currency (2026-09-22); closet-mode figures stay in dollars.
  const PREFIX = { USD: '$', CAD: 'CA$', GBP: '£', EUR: '€', AUD: 'A$' };
  const dollars = (c, cur) => (PREFIX[cur || 'USD'] || (cur + ' ')) + (Math.round(c || 0) / 100).toLocaleString('en-US', { maximumFractionDigits: 2 });
  ...
  async function renderCentre(id) {
    ...
    el.innerHTML = `<p><a href="#centres" data-action="tab" data-tab="centres">Centres</a> › ${esc(c.name)}</p>
      ...
      <div class="detail show" data-ctab="overview">
        ${d.balance ? `<div class="card"><b>Link mode</b> · ${esc(c.currency || 'USD')} · balance ${dollars(d.balance.balanceCents, c.currency)} · raised ${dollars(d.balance.raisedCents, c.currency)} · redeemed ${dollars(d.balance.redeemedCents, c.currency)} · goal ${dollars(c.goal_cents, c.currency)} · ${d.balance.orders} orders, ${d.balance.sponsors} sponsors<br><span class="soft">Redeem and adjust with the vc_redeem tool. Notification email: ${esc(c.statements_email || '')}</span></div>` : ''}
